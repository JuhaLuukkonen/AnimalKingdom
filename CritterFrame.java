...
public class CritterFrame extends JFrame {
    ...
    public CritterFrame(int width, int height) {
        // tämä estää tekemästä kopiota GUI komponentista
        if (created)
        ...
        // käynnistymisen epäonnistuminen
        started = false;
    }

    // kontrollirakenne ja nimike alas paneelille
    private void constructSouth() {
        // lisää aika kontrolli alas
        JPanel p = new JPanel();
    ...
    // aloittaa simulaation...olettaa että kaikki oliot on jo lisätty
    public void start() {
        // älä anna kenenkään aloittaa toistamiseen ja muista jos on jo aloitettu
        if (started) {
            return;
        }
        // jos ei lisätty olioita, ei tehdä mitään
    ...
    // lisää oikea palsta näyttääkseen kuinka monta oliota on elossa
    ...
    // lisää tietty numero oliolle siihen liittyvästä syystä
    public void add(int number, Class<? extends Critter> c) {
        // älä anna muiden olioiden aloittaa
        if (started) {
            return;
        }
